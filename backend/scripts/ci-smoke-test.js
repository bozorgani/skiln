...
const main = async () => {
  ...
  const categoryResponse = await jsonFetch('/categories', {
    method: 'POST',
    headers: { Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ name: `CI Category ${suffix}`, type: 'course', order: 1 }),
  });
  assert.ok(categoryResponse.data.category._id);

  const contactResponse = await jsonFetch('/contact/messages', {
    method: 'POST',
    body: JSON.stringify({
      name: 'CI Contact',
      email: `ci-${suffix}@example.com`,
      subject: 'CI smoke contact',
      message: 'This is a contact message generated by CI smoke test.',
    }),
  });
  assert.ok(contactResponse.data.message._id);
  ...
  const reviewResponse = await jsonFetch(`/courses/${course._id}/reviews`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${studentToken}` },
    body: JSON.stringify({ rating: 5, title: 'CI review', content: 'Great course from CI smoke test.' }),
  });
  assert.ok(reviewResponse.data.review._id);

  await jsonFetch(`/reviews/${reviewResponse.data.review._id}/moderate`, {
    method: 'PUT',
    headers: { Authorization: `Bearer ${adminToken}` },
    body: JSON.stringify({ isApproved: true }),
  });

  const reviews = await jsonFetch(`/courses/${course._id}/reviews`);
